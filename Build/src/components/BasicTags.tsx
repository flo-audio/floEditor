...
const INPUT_GROUPS: Array<{
  ...
  },
  {
    ...
    fields: [
      { key: "track_number", label: "Track Number", placeholder: "04" },
      { key: "track_total", label: "Total Tracks", placeholder: "12" },
      { key: "disc_number", label: "Disc Number", placeholder: "1" },
      { key: "disc_total", label: "Total Discs", placeholder: "2" },
      { key: "bpm", label: "BPM", placeholder: "118", type: "number" },
      { key: "key", label: "Key", placeholder: "A Minor" }, // Fixed
      {
        key: "isrc",
        label: "ISRC",
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "encoded_by", label: "Encoded By", placeholder: "FloEditor" }, // Fixed
    ],
  },
  {
    title: "Links",
...
export function BasicTagsSection({ metadata, onMetadataChange }: BasicTagsSectionProps) {
  const populatedFields = Object.entries(metadata).filter(([, value]) =>
    Boolean(value && (typeof value === "string" ? value.trim() : true)),
  ).length;
  const totalFields =
  ...
  const renderInput = ({
    ...
    const stringValue = typeof value === "string" || typeof value === "number" ? value : ""; // Ensure string/number
    return (
      <div
        key={key}
      ...
      >
        ...
        <input
          id={key as string}
          value={stringValue}
          onChange={(e) => onMetadataChange(key, type === "number" ? parseInt(e.target.value) || undefined : e.target.value)}
    ...
    <section className="glass-panel p-6 mb-6">
      ...
      <div className="mt-8 space-y-2">
        ...
        <textarea
          ...
          onChange={(e) => onMetadataChange("comments", [{ text: e.target.value }])}
